package cn.donespeak.protobufvalidation.interceptor;
	...
	protected void validate(GeneratedMessageV3 messageV3) {
		for (Descriptors.FieldDescriptor fieldDescriptor : messageV3.getDescriptorForType().getFields()) {
			if (messageV3.getField(fieldDescriptor) instanceof GeneratedMessageV3) {
				...
				doValidate(fieldDescriptor.getFile().getFullName(), fieldDescriptor.getFullName(),
						messageV3.getField(fieldDescriptor), fieldDescriptor.getOptions());
			}
		}
	}

	private void doValidate(String protoName, String fieldName, Object fieldValue,
			DescriptorProtos.FieldOptions options) throws IllegalArgumentException {
